...
// Generate a simple PDF statement using the Print API — no extra library needed
  ...
  const html = `
    ...
    <body>
      ...
      <div class="section">
        ...
        ${accounts.map(a => `
          <div class="row">
            <span>${a.account_type === "savings" ? "Savings" : "Current"} · ${a.account_number}</span>
      ...
      <div class="footer">
        This statement was generated by SmartBank AI · Confidential · For personal use only
      </div>
    </body>
    </html>
  `;
...
export default function Profile() {
  ...
  async function handleSaveProfile(e) {
    ...
    try {
      ...
      // Backend route not built — still show success for demo
      const updated = { ...user, name, email };
      const storage = localStorage.getItem("token") ? localStorage : sessionStorage;
      storage.setItem("user", JSON.stringify(updated));
      setUser(updated);
